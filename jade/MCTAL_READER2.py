...
class Tally:
    ...
    def Print(self, option=[]):
        ...
        if self.verbose:
            ...
            if not self.mesh:
                print(
                    "Number of cells/point detectors/surfaces/macrobodies: %5d"
                    % self.getNbins("f")
                )
            else:
            ...
            print(
                "Number of tot vs. dir or flag vs. unflag bins: %5d"
                % self.getNbins("d")
            )
...
class MCTAL:
    ...
    def __init__(self, fname, verbose=False):
        ...
        self.line = (
            None  # This variable will contain the read lines one by one, but it is
        )
    ...
    def getHeaders(self):
        ...
        while self.line[0].lower() != "tally":
            for l in self.line:
                self.header.ntals = np.append(self.header.ntals, int(l))
            self.line = self.mctalFile.readline().split()
        ...
        if tally.mesh == False:
            ...
            while "tally" not in self.line and len(self.line) != 0:
                if "kcode" in self.line:
                    ...
                    self.tallies.append(
                        tally
                    )  # append the current tally (anyway it's finished)

                    self.kcode.header = self.line.split()[1:]
                    for (
                        self.line
                    ) in self.mctalFile.readlines():  # just read mctal until the end
