...
def executeBenchmarksRoutines(session, lib):
    ...
    for testname, row in config.iterrows():
        ...
        if bool(row['OnlyInput']) or bool(row['Run']):
            print('        -- '+testname.upper()+' TEST STARTED --\n')
            print(' Generating input files:'+'    ' +
                  str(datetime.datetime.now()))
            log.adjourn(testname.upper()+' test run started' + '    ' +
                        str(datetime.datetime.now()))
            ...
            if bool(row['OnlyInput']):
                ...
                print('\n        -- '+testname.upper()+' TEST COMPLETED --\n')
                # Adjourn log
                log.adjourn(testname.upper()+' test run completed with success'
                            + '    '+str(datetime.datetime.now()))
