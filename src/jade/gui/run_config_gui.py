...
try:
    ...
    logging.info(
        "Warning: 'tkinter' is not installed. GUI functionality will be disabled."
    )
    tk = None
    TclError = Exception
try:
    ...
    ThemedTk = None
    logging.info(
        "Warning: 'ttkthemes' is not installed. GUI functionality will be disabled."
    )
...
if tk:
    ...
    class ConfigGUI:
        ...
        def create_benchmarks_tab(self):
            ...
            self.bench_column_mapping = {
                f"#{i+1}": col for i, col in enumerate(columns)
            }
...
if __name__ == "__main__":
    if tk:
        ...
        logging.info("The GUI cannot be launched because 'tkinter' is not installed.")
